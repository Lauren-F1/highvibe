...
const mockHostSpaces: HostSpace[] = [
  ...
  { id: 'space2', name: 'Sacred Valley Hacienda', location: 'Cusco, Peru', capacity: 18, rate: 1500, status: 'Published', bookings: 5, image: getPlaceholderById('hacienda-leather-interior'), hostLat: -13.53, hostLng: -71.96,
    description: 'A historic hacienda in the heart of the Andes, offering a unique blend of culture and comfort.',
    amenities: [],
    propertyShowcaseUrls: Array(4).fill(getPlaceholderById('hacienda-leather-interior')),
    availabilitySet: true,
  },
...
export default function HostDashboardPage() {
  ...
  const handleRequestConnection = async (targetProfile: Guide | Vendor) => {
    ...
    try {
      ...
      toast({ title: "Connection Request Failed", description, variant: "destructive" });
    } finally {
    ...
    <div className="container mx-auto px-4 py-8 md:py-12">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        ...
        <div className="relative h-[104px] w-[420px] hidden lg:block mx-auto flex-shrink-0 rounded-xl shadow-md overflow-hidden">
            <Image src={hostHeroImage} alt={'Cozy cabin in forest'} fill className="object-cover" style={{ objectPosition: 'center 40%' }} />
        </div>
            ...
            <CardHeader className="!pb-2">
                ...
                <div className="!mt-4 max-w-sm">
                    <Select onValueChange={setActiveSpaceId} value={activeSpaceId || ''}>
                        <SelectTrigger id="space-selector" className="text-lg py-6"><SelectValue placeholder="Select a space to see matches..." /></SelectTrigger>
                        <SelectContent>{hostSpaces.map(space => (<SelectItem key={space.id} value={space.id}>{space.name}</SelectItem>))}</SelectContent>
                    </Select>
                </div>
            </CardHeader>
                            ...
                            <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'guides' | 'vendors')}>
                                ...
                                <TabsContent value="guides" className="mt-6">
                                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                                        {guideFiltersVisible && (<div className="lg:col-span-1"><GuideFilters filters={guideFilters} onFiltersChange={handleGuideFilterChange} onApply={handleApplyGuideFilters} onReset={handleResetGuideFilters} isDirty={guideFiltersDirty}/></div>)}
                                        <div className={cn(guideFiltersVisible ? 'lg:col-span-3' : 'lg:col-span-4')}>
                                            <div className='space-y-4'>
                                                <div className="flex justify-between items-center mb-4 gap-4">
                                                    <div className='flex items-center gap-4'>
                                                        <Button onClick={() => setGuideFiltersVisible(!guideFiltersVisible)} variant="outline"><Filter className="mr-2 h-4 w-4" />{guideFiltersVisible ? 'Hide' : 'Show'} Filters{!guideFiltersVisible && appliedGuideFiltersCount > 0 && (<Badge variant="secondary" className="ml-2">{appliedGuideFiltersCount}</Badge>)}</Button>
                                                ...
                                                {displayedGuides.length > 0 ? (
                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">{displayedGuides.map(guide => <GuideCard key={guide.id} guide={guide} onConnect={handleRequestConnection} onViewMessage={handleViewMessage} connectionStatus={getPartnerStatus(guide.id)}/>)}</div>
                                                ) : (<Card className="text-center py-12"><CardHeader><CardTitle className="font-headline text-xl">No matches yet.</CardTitle></CardHeader><CardContent><Button onClick={handleResetGuideFilters}>Reset Filters</Button></CardContent></Card>)}
                                            </div>
                                        </div>
                                    </div>
                                </TabsContent>
                                <TabsContent value="vendors" className="mt-6">
                                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                                        {vendorFiltersVisible && (<div className="lg:col-span-1"><VendorFilters filters={vendorFilters} onFiltersChange={handleVendorFilterChange} onApply={handleApplyVendorFilters} onReset={handleResetVendorFilters} isDirty={vendorFiltersDirty}/></div>)}
                                        <div className={cn(vendorFiltersVisible ? 'lg:col-span-3' : 'lg:col-span-4')}>
                                            <div className="space-y-4">
                                                <div className="flex justify-between items-center mb-4 gap-4">
                                                    <div className='flex items-center gap-4'>
                                                         <Button onClick={() => setVendorFiltersVisible(!vendorFiltersVisible)} variant="outline"><Filter className="mr-2 h-4 w-4" />{vendorFiltersVisible ? 'Hide' : 'Show'} Filters{!vendorFiltersVisible && appliedVendorFiltersCount > 0 && (<Badge variant="secondary" className="ml-2">{appliedVendorFiltersCount}</Badge>)}</Button>
                                                ...
                                                {displayedVendors.length > 0 ? (
                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">{displayedVendors.map(vendor => <VendorCard key={vendor.id} vendor={vendor} distance={vendor.distance} onConnect={handleRequestConnection} onViewMessage={handleViewMessage} connectionStatus={getPartnerStatus(vendor.id) as any}/>)}</div>
                                                ) : (<Card className="text-center py-12"><CardHeader><CardTitle className="font-headline text-xl">No matches yet.</CardTitle></CardHeader><CardContent><Button onClick={handleResetVendorFilters}>Reset Filters</Button></CardContent></Card>)}
                                            </div>
                                        </div>
                                    </div>
                                </TabsContent>
