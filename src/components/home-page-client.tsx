    ...
    <Link href={href} className="group">
      <Card className="h-full w-full transition-all duration-300 ease-in-out hover:shadow-xl hover:border-primary">
        ...
        <CardContent className="text-center p-6 pt-0">
          <CardDescription className="text-lg font-body">{description}</CardDescription>
        </CardContent>
      </Card>
    </Link>
  );
...
export default function HomePageClient() {
  return (
    <main className="flex min-h-screen w-full flex-col items-center bg-background p-4 pt-4 sm:p-6 md:p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 w-full max-w-7xl">
        <RoleCard
          href="/seeker"
          icon={<SeekerIcon className="w-44 h-44 text-primary" />}
          title="I'm a Seeker"
        ...
        <RoleCard
          href="/host"
          icon={<SpaceOwnerIcon className="w-36 h-36 text-primary" />}
          title="I'm a Host"
